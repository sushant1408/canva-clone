...
const useHistory = ({ canvas, saveCallback }: UseHistoryProps) => {
  ...
  const undo = useCallback(() => {
    if (canUndo()) {
      ...
      canvas?.loadFromJSON(previousState).then((restoredCanvas) => {
  ...
  const redo = useCallback(() => {
    if (canRedo()) {
      ...
      canvas?.loadFromJSON(nextState).then((restoredCanvas) => {
        restoredCanvas.renderAll();
  ...
  return { canRedo, canUndo, redo, undo, save, setHistoryIndex, canvasHistory };
};
